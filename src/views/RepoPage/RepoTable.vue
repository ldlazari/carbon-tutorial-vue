  ...
  <cv-data-table :columns="columns" :title="title" :helper-text="helperText">
    <template slot="data">
      ...
      <!-- <cv-data-table-row v-for="row in rows" :key="`${row.id}`"> -->
        ...
        <cv-data-table-cell>{{ row.updatedAt }}</cv-data-table-cell>
        <cv-data-table-cell>{{ row.issueCount }}</cv-data-table-cell>
        <cv-data-table-cell>{{ row.stars }}</cv-data-table-cell>
        <cv-data-table-cell>{{ row.links }}</cv-data-table-cell> -->
